...
**Question**: Write a query that returns the total number of leads submitted by **guest users** (those with `NULL` in `user_id`) and **registered users** for each listing. Additionally, the query should return the name of the user who submitted each lead, and for guest users, it should display `"Guest"` instead of `NULL`.
